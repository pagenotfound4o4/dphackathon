    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            holder.name = (TextView) convertView
                    .findViewById(R.id.txt_name_content);
            holder.address = (TextView) convertView
                    .findViewById(R.id.txt_address_content);
        ...
        if (position == 0) {
            ...
            params.addRule(RelativeLayout.ALIGN_BOTTOM, R.id.txt_content);
        } else { // 不是第一条数据
            // 本条数据和上一条数据的时间戳相同，时间标题不显示
            if (mItems.get(position).getDate()
                    .equals(mItems.get(position - 1).getDate())) {
                holder.title.setVisibility(View.GONE);
                params.addRule(RelativeLayout.ALIGN_TOP, R.id.txt_content);
